...
void LinearChargeInterpolation(type_double rho[], const type_double z[], const type_double r[], const Grid& grid,
                               type_double charge, const size_t Ntot) {
    int cell_z, cell_r, Nr=grid.Nr;
    type_double hz, hr;
    #pragma omp parallel private(hz, hr, cell_z, cell_r, charge) num_threads(NUM_THREADS)
    {
    #pragma omp for
    ...
    for (int i = 0; i < Ntot; i++) {
        ...
        rho[cell_z*Nr + cell_r] += charge * (1 - hz) * (1 - hr);
        rho[(cell_z+1)*Nr + cell_r] += charge * hz * (1 - hr);
        rho[(cell_z+1)*Nr + cell_r+1] += charge * hz * hr;
        rho[cell_z*Nr + cell_r+1] += charge * (1 - hz) * hr;
    }
    }
}
